...
class ModelMemEstimator:
    ...
    def calculate_emb_mem(self, bit: int = 16) -> float:
        ...
        return (
            convert_to_unit(mem_embed, unit),
            f"{convert_to_unit(mem_embed, unit)} {unit}",
        )
    ...
    def calculate_single_selfattn_mem(self):
        ...
        weight_size = (
            4 * self.h1 * self.h1 * 4
        )  # 4 means fp32 storage weight has 4 bytes
        return weight_size
        ...
        for layer, config in partition.items():
            ...
            for idx, value in enumerate(shard):
                if value == 0:
                    # add the kv size
                    kv_size = (
                        self.calculate_single_layer_maximum_kv_cache()
                    )  # calculate in fp16
                    # bits
                    bit = bits[idx]
                    if bit == "8:tc":  # only for tensorcore, we store the kv in INT8
                        ...
                        kv_size = (
                            kv_size * bit / 16
                        )  # the feature supported by pure int8
                    all_size_estimation += kv_size
        return (
            convert_to_unit(all_size_estimation, unit),
            f"{convert_to_unit(all_size_estimation, unit)} {unit}",
        )
    ...
    def calculate_temp_tensor_size_prefill(self, unit="b"):
        ...
        bmm_tmp_size = (
            self.b * self.s * self.s + self.b * self.s * self.h1
        ) * 2  # by default to 16
        ...
        return (
            convert_to_unit(total_tmp_size, unit),
            f"{convert_to_unit(total_tmp_size, unit)} {unit}",
        )
    ...
    def calculate_temp_tensor_size_next_i(self, unit="b"):
        ...
        # 2 BMM (for qk_bmm, there is a softmax)
        bmm_tmp_size = (
            self.b * (self.s + self.n) + self.b * self.h1
        ) * 2  # by default to 16
        # 32
        softmax_tmp_size = self.b * (self.s + self.n) * 4
        # tmp buffer for kv cache
        kv_cache_tmp = (
            2 * (self.b * (self.s + self.n) * self.h1) * 2
        )  # by default to 16
        # ffn
        # a layer norm
    ...
    def calculate_temp_tensor_size(self, unit="b"):
        max_temp = max(
            self.calculate_temp_tensor_size_prefill(unit)[0],
    ...
    def calculate_temp_tensor_size_with_bz(self, bz_prefill, bz_decode, unit="b"):
        ...
        # return in bytes
        max_temp = max(
            self.calculate_temp_tensor_size_prefill(unit)[0] * ratio_prefill,
    ...
    def estimate_single_layer_kv_cache(self, unit="b"):
        ...
        return (
            self.calculate_single_layer_maximum_kv_cache(),
            f"{convert_to_unit(self.calculate_single_layer_maximum_kv_cache(), unit)} {unit}",
        )


def estimate_single_layer_mem(
    model_mem_estimator: ModelMemEstimator, shards: List = [0], bits: List[int] = [16]
):
    partition = {0: {"shard": shards, "bits": bits}}
    mem_require, _ = model_mem_estimator.calculate_maximum_mem_occupation_of_partition(
        partition, unit=PQConfig.MEM_UNIT
    )
    return mem_require
...
def get_device_topo_available_mem_with_order(
    ...
    M_d = np.array(
        [
            get_single_device_mem_constraints(device_name) * cnt
            for d_rank, (device_name, cnt) in device_topo.items()
        ]
    )
    ...
    temp_tensor_mem = model_mem_estimator.calculate_temp_tensor_size_with_bz(
        prefill_bz, bz_decode_max, unit=mem_unit
    )[0]
...
def estimate_single_device_mem(layers_range, bit_assignment, model_mem_estimator):
    ...
    # k % 2 means shard
    for k in range(i, j):
        layer_x_mem = estimate_single_layer_mem(
            model_mem_estimator, 0, bit_assignment[k * 2]
...
def check_memory_budget_single_device(
    device_mem, device_rank, layers_range, bit_assignment, model_mem_estimator
):
    i_to_j_mem = estimate_single_device_mem(
        layers_range, bit_assignment, model_mem_estimator
    )
    if i_to_j_mem > device_mem:
        print(
            f"memory budget exceeded for device {device_rank}, {i_to_j_mem} > {device_mem}"
        )
        return False
    return True
...
def check_memory_budget(res, model_mem_estimator, name="llm_pq"):
    ...
    D_mem = get_device_topo_available_mem_with_order(
        D, model_mem_estimator, prefill_bz, bz_decode_max
    )
    for device_rank, layers_range in partition_result.items():
        device_mem = D_mem[device_rank]
        flag = check_memory_budget_single_device(
            device_mem, device_rank, layers_range, bit_assignment, model_mem_estimator
        )
        if not flag:
