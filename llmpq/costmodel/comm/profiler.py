...
def build_dummy(
    batch_size: int, hidden_space: int, sample_num: int
) -> Tuple[torch.Tensor, Dict[int, torch.Tensor]]:
    """
    build dumpy data for profiling communication cost of each worker (device) pair
    """
    __DEVICE__INDEX__ = llmpq._globals.__DEVICE__INDEX__
    __PIPELINE__MODEL__PARALLEL__GROUP__ = (
        llmpq._globals.__PIPELINE__MODEL__PARALLEL__GROUP__
    )
    ...
    for i in range(dist.get_world_size()):
        ...
        for idx_data in range(sample_num):
            data[idx_data] = torch.rand(
                bzz[idx_data % len(bzz)],
                interested_i[idx_data % len(interested_i)],
                hidden_space + 1024 * (idx_data % 3),
                device=device,
            )
        send_dummy[i] = data
...
def generate_sender(
    send_dumpy: Dict[int, List[Tensor]], warmup: int
) -> Tuple[Dict[str, List[Tensor]], Dict[str, List[float]]]:
    """
            ...
            for n in range(len(dummy_data)):
                ...
                for epoch in range(1, 3 * warmup):
                    ...
                    if epoch > warmup:
                        ...
                        print("time")
                if device != "cpu":
                    data_size_buffer[f"{rank}_{i}"].append(
                        get_size_cuda(dummy_data[n], unit="MB")
                    )
                else:
                    data_size_buffer[f"{rank}_{i}"].append(
                        get_size_cpu(dummy_data[n], unit="MB")
                    )  # MB
                time_buffer[f"{rank}_{i}"].append(
                    sum(1000 * avg_time) / len(avg_time)
                )  # ms
            data_size_buffer[f"{rank}_{i}"] = torch.tensor(
                data_size_buffer[f"{rank}_{i}"], device=device
            )
            time_buffer[f"{rank}_{i}"] = torch.tensor(
                time_buffer[f"{rank}_{i}"], device=device
            )
    # sync
    if __PIPELINE__MODEL__PARALLEL__GROUP__ is not None:
...
def generate_cost_model_dataset(
    batch_size: int, hidden_space: int, sample_num: int, warmup: int
...
def fit_cost_model(
    dataset: Tuple[Dict[str, List[Tensor]], Dict[str, List[float]]],
) -> Dict[str, np.ndarray]:
    """
