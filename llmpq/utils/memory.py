...
def get_size_cuda(model_or_tensor, unit="bytes"):
    if isinstance(model_or_tensor, torch.Tensor):
        ...
        total_params = sum(
            p.element_size() * reduce(mul, p.shape, 1) for p in params if p.is_cuda
        )
        total_buffers = sum(
            b.element_size() * reduce(mul, b.shape, 1)
            for b in model_or_tensor.buffers()
            if b.is_cuda
        )
...
def get_size_cpu(model_or_tensor, unit="bytes"):
    if isinstance(model_or_tensor, torch.Tensor):
        ...
        params = list(model_or_tensor.parameters())
        total_params = sum(
            p.element_size() * reduce(mul, p.shape, 1) for p in params if not p.is_cuda
        )
        total_buffers = sum(
            b.element_size() * reduce(mul, b.shape, 1)
            for b in model_or_tensor.buffers()
