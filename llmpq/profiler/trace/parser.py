...
def parse_module_avg_cost(module_cnt: Dict[str, int], trace_path: str) -> Dict[str, Any]:
    ...
    def match_kernel_ranges():
        ...
        with tqdm(total=total_events, desc="Matching kernel ranges", unit="module_event") as pbar:
            for module_name, events in module_event_dict.items():
                for e_id, event in enumerate(events):
                    ...
                    dur = event['dur']
                    start, end = ts, ts + dur
                    tgt_events = [e for e in cudaLaunchKernelEvents if start <= e['ts'] <= end]
    ...
    def process_kernels(module_event_to_cudaLaunchKernelEvents):
        ...
        total_kernels = sum(len(v) for v in module_event_to_cudaLaunchKernelEvents.values())
        with tqdm(total=total_kernels, desc="Processing kernels", unit="kernel") as main_pbar:
            for module_name, cuda_events in module_event_to_cudaLaunchKernelEvents.items():
                with tqdm(cuda_events, desc=f"Matching {module_name}", leave=False) as sub_pbar:
                    for cuda_event in sub_pbar:
                        matched = []
                        find_one = False
                        with tqdm(kernel_events, desc="Scanning kernels", leave=False) as kernel_pbar:
                            for event in kernel_pbar:
                                try:
                                    if event['args']['correlation'] == cuda_event['args']['correlation']:
                                        find_one = True
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future2 = executor.submit(process_kernels, module_event_to_cudaLaunchKernelEvents)
    ...
    def process_run_engine_events():
        ...
        with tqdm(enumerate(run_engine_events), total=len(run_engine_events), desc="Processing runs") as pbar:
    ...
    def aggregate_results(run_e_name_pd_mapping):
        ...
        total_ops = sum(len(pd) for run in run_e_name_pd_mapping.values() for pd in run.values())
        with tqdm(total=total_ops, desc="Aggregating results") as main_pbar:
            for run_id, pd_mapping in run_e_name_pd_mapping.items():
                with tqdm(pd_mapping.items(), desc=f"Run {run_id}", leave=False) as sub_pbar:
                    for phase, e_names in sub_pbar:
                        for e_name in e_names:
                            module = next((m for m in modules_names if m in e_name), None)
                            assert module, f"Module not found in {e_name}"
                            if phase not in cost_table[module]:
                                cost_table[module][phase] = []
                            cost_table[module][phase].extend(module_cuda_kernel_dur[e_name])
                            main_pbar.update(1)
        return cost_table
