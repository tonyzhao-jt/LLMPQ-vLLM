...
def solve_ilp_pulp(L, N, BITs, M, M_d, l, comm):
    ...
    z = pulp.LpVariable.dicts(
        "z", [(i, j) for i in range(L) for j in range(N)], cat=pulp.LpBinary
    )
    LAT = pulp.LpVariable.dicts(
        "LAT", [j for j in range(N)], lowBound=0, cat=pulp.LpContinuous
    )
...
def main(args):
    ...
    assert (
        chosen_bit in available_bits
    ), f"chosen bit {chosen_bit} not in available bits {available_bits}"
    ...
    L, N, BITs, M_d, M, l, comm = prepare_for_ilp(
        num_hidden_layers, D, chosen_bit, cost_model_pack, bz_pack
    )
