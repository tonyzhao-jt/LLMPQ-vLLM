...
def get_device_e2e_lat(
    ...
    bits = [
        (bit_assignment[2 * layer_idx], bit_assignment[2 * layer_idx + 1])
    ...
    decode_lat = stage_pure_exe_latency(
        D_name,
        bits,
        lat_cost_model,
        bz_decode_max,
...
def check_latency(
    ...
    for rank, layer_range in partition_result.items():
        prefill_lat, decode_lat = get_device_e2e_lat(
            rank, partition_result, bit_assignment, current_D, lat_cost_model, params
        )
...
def change_partition_result(
    partition_result: dict, pioneer_rank, straggler_rank, num_convert
):
    copied_partition_result = copy.deepcopy(partition_result)
    # each rank value
    for rank, value in partition_result.items():
        if rank == pioneer_rank:
            copied_partition_result[rank] = (
                value[0],
                value[1] + num_convert,
...
def exchange_precisions(
    original_bit_assignment: dict,
    pioneer_layer_idx: int,
    straggler_idxs: list,
    conversion: list,
):
    ...
    original_bitwidths = [
        original_bitwidths_values[idx * 2] for idx in range(layer_num)
    ]
    # minmax
    ...
    for insert_idx in range(num_convert):
        ...
        if (
            pioneer_layer_idx < min_straggler_layer_idx
        ):  # pioneer layer is before the straggler
            ...
            while ref_idx < straggler_idxs_len:
                ...
                try:
                    try_insert_idx = (
                        strag_layer_idx + (num_convert - 1) + pop_idx
                    )  # new idx
            ...
            pioneer_layer_idx > max_straggler_layer_idx
        ):  # pioneer layer is after the straggler
            copied_bit_assignment.pop(pioneer_layer_idx)
            for _ in range(num_convert):
                copied_bit_assignment.insert(
                    pioneer_layer_idx - num_convert, straggler_precs
                )
            # pop all straggler precisions
...
def check_new_plan_valid(old_partition, old_bit_ass, new_partition, new_bit_ass):
...
def calculate_stage_latency(
    ...
    for device_rank, device_name in current_D.items():
        prefill_stage_lat, decode_stage_lat = get_device_e2e_lat(
            device_rank,
            partition_result,
            bit_assignment,
            current_D,
        ...
        stage_time = (
            prefill_stage_lat * (prefill_micro_bs_num - 1)
            + (mu_n - 1) * (decode_micro_bs_num - 1) * decode_stage_lat
        )
        ...
        while True:
            ...
            for pioneer in candidate_pioneers:
                straggler_rank = straggler[
                    1
                ]  # always stragler, if you cannot speed up straggler, you can never speed up
                ...
                for pioneer_prec, (strag_prec, num_required) in convert_pairs:
                    ...
                    if device_info_dict[pioneer_rank][pioneer_prec]["sum"] <= 0:
                        continue
                    if (
                        device_info_dict[straggler_rank][strag_prec]["sum"]
                        <= num_required
                    ):
                        continue
                    # available, get pairs
                    pioneer_prec_layers = device_info_dict[pioneer_rank][pioneer_prec][
                        "layers"
                    ]
                    straggler_prec_layers = device_info_dict[straggler_rank][
                        strag_prec
                    ]["layers"]
                    possible_straggler_layer_pairs = get_combinations(
                        straggler_prec_layers, num_required
                    )

                    conversion = (pioneer_prec, strag_prec)
                    new_partition_result = change_partition_result(
                        partition_result, pioneer_rank, straggler_rank, num_required
                    )
                    ...
                    for pioneer_layer_idx in pioneer_prec_layers:
                        ...
                        for straggler_layer_idxs in possible_straggler_layer_pairs:
                            ...
                            candidates = exchange_precisions(
                                bit_assignment,
                                pioneer_layer_idx,
                                straggler_layer_idxs,
                                conversion,
                            )
                            ...
                            for candidate in candidates:
                                ...
                                if obj1_new is None:
                                    try:
                                        ...
                                        pdb.set_trace()
                                obj2_new = (
                                    check_bitwidth_omega(
                                        candidate, available_bits, omega
                                    )
                                    * theta
                                )
                    ...
                    if new_obj is not None and new_obj < optimal_obj:
                        ...
                        strag_new_mem = estimate_single_device_mem(
                            straggler_layers_range,
                            better_bit_assignment,
                            model_mem_estimator,
                        )
                        pioneer_new_mem = estimate_single_device_mem(
                            pioneer_layers_range,
                            better_bit_assignment,
                            model_mem_estimator,
                        )
                        ...
                        if (
                            strag_new_mem > strag_D_mem
                            or pioneer_new_mem > pioneer_D_mem
                        ):
                ...
                if better_candidate is not None:
                    check_new_plan_valid(
                        partition_result,
                        bit_assignment,
                        better_candidate["new_sol"]["partition_result"],
                        better_candidate["new_sol"]["bit_assignment"],
                    )
                    # check the result
                    print("Update")
                    layer_change_candidate = better_candidate["layer_changed"]
                    straggler_rank, pioneer_rank = better_candidate["involved_devices"]
                    new_partition_result = better_candidate["new_sol"][
                        "partition_result"
                    ]
...
def enumerate_best_result(args):
    ...
    for perm in permutations:
        ...
        for prefill_bz in candidate_prefill_bzs:
            ...
            res = llm_pq_h_internal_main(
                num_hidden_layers, cost_model_pack, bz_pack, current_D
            )
