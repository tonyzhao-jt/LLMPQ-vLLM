...
from llmpq.optimizer.utils import assign_uniform_bit
...
from ..partitioner.helper import (create_device_mesh_and_mem,
                                  decouple_result_group, get_device_info,
                                  init_parameters_and_cost_models)
from ..partitioner.utils import (estimate_min_max_mem,
                                 interpret_ilp_result_i_j_b)
from ..utils import (get_available_bits_pair, get_default_decode_bz,
                     partition_a_into_b_bins, save_with_pickle,
                     to_weight_int8_if_tc_not_available)
from .mem_utils import (get_device_topo_available_mem_with_order,
                        get_M_with_bitwidth_pair)
from .utils import NOT_AVAILABLE, create_ilp_solver
...
def prepare_for_ilp(num_hidden_layers, D, available_bits, bz_pack, model_mem_estimator):
    ...
    #         new_BITs = get_available_bits_pair(PQConfig.AVAILABLE_BITS_WO_INFO)
...
def main(args):
    ...
    L, N, BITs, M_d, M, omega = prepare_for_ilp(
        num_hidden_layers, D, available_bits, bz_pack, model_mem_estimator
    )
